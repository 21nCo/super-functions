...
async mergeRecord(
  ...
  // Begin a real database transaction or optimistic-concurrency operation.
  const existing = await this.getRecord(resource, id);
  if (!existing && options?.ifMissing) {
    // ifMissing is the complete create payload. Insert it atomically only if
    // the row is still absent; do not merge partial over it.
    const created = { ...options.ifMissing, id };
    await this.upsertRecord(resource, created);
    // Commit transaction
    return created;
  }
