...
async mergeRecord(
  ...
  // Adapt these transaction APIs to your backend. The isolation/row lock (or
  // an equivalent compare-and-swap) is required; a plain read + upsert races.
  return this.db.transaction({ isolation: "serializable" }, async (tx) => {
    const existing = await tx.getRecordForUpdate(resource, id);
    if (!existing && options?.ifMissing) {
      const created = { ...options.ifMissing, id };
      await tx.insertRecord(resource, created); // unique key rejects a second creator
      return created;
    }
