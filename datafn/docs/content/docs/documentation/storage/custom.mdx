...
The `mergeRecord` method must be atomic across every adapter instance: read the existing record, merge fields, and write back in a single backend transaction. If transactions are unavailable, use a distributed lock with fencing, versioned compare-and-set, or a mutex shared by every instance; a process-local per-instance mutex is insufficient. A row lock cannot lock an absent row, so adapters must retry the complete transaction after a retryable serialization or duplicate-key conflict. The retry re-reads the winner and applies the losing caller's `partial` update.
