...
| Relation names valid | Relation tokens in `select` and `filters` must reference defined relations, including [polymorphic](/docs/dfql/schema/relations#polymorphic-relations) and inverse relations. |
| Select tokens parseable | Each token in `select` is parsed and validated (e.g., `"assignee.*"`, `"tags.#"`, `"items.*#"`). |
| Temporal clause valid | A [`temporal`](/docs/dfql/temporal) clause must target a readable field and use a valid `scale` (`hour`/`day`/`week`/`month`/`quarter`/`year`) and `storage`. |
...
### Hierarchical (htree) and Polymorphic Relations

Htree traversal is validated against the schema rather than hardcoded field names: the token must match an `htree` relation on the resource, and the resource must define the relation's `pathField` (default `parentPath`). A query that uses htree on a resource without a matching relation is rejected with `DFQL_UNKNOWN_RELATION`. Nested select/filter validation through polymorphic inverse relations is checked across every concrete target resource.
