...
DataFn search is provider-backed and local-first when a `searchProvider` is configured on the client. Which provider you use depends on topology:

- Browser-owned apps typically use a JavaScript SearchFn provider backed by IndexedDB.
- Native-backed Apple WebView apps use `createNativeBackedSearchProvider(...)`, which routes search into Swift and the shared native SearchFn backend.
- Native-backed CloudKit apps use the same Swift backend; CloudKit syncs records, not search index files.
...
|---|---|---|
| `query` | `string` | Search text. |
| `resources` | `string[]` | Resources to search. Defaults to all searchable resources. |
| `fields` | `string[]` | Optional subset of indexed fields. |
| `filters` | `Record<string, Record<string, unknown>>` | Per-resource DFQL filters applied on top of the candidate IDs. |
| `temporal` | `DatafnTemporalClause \| DatafnTemporalClause[]` | Timezone-aware temporal filter applied to every searched resource. |
| `temporalByResource` | `Record<string, DatafnTemporalClause \| DatafnTemporalClause[]>` | Per-resource temporal filters. |
| `select` | `string[]` | Fields to return on hits. |
| `limit` | `number` | Maximum total results. |
| `limitPerResource` | `number` | Maximum results per resource. |
| `prefix` | `boolean` | Prefix token matching. |
| `fuzzy` | `boolean \| number` | Fuzzy matching (or distance/tolerance hint). |
| `fieldBoosts` | `Record<string, number>` | Field-specific relevance weights. |
| `source` | `"auto" \| "local" \| "remote"` | Execution source (see below). |
| `signal` | `AbortSignal` | Cancel an in-flight search. |

### Temporal Filtering

Search accepts [temporal](/docs/dfql/temporal) clauses, which are compiled to per-resource date filters before execution. Use `temporal` to apply the same clause to every resource, or `temporalByResource` to target specific resources.
...
const result = await client.search({
  ...
  resources: ["sessions"],
  temporalByResource: {
    sessions: time.day("startedAt", "2026-05-18T12:00:00.000Z", { timezone: "user" }),
  },
});
```
...
```json
{
  ...
  "fieldBoosts": { "text": 2, "name": 1 },
  "temporalByResource": {
    "todos": { "field": "dueAt", "period": { "scale": "month" }, "timezone": "user" }
  },
  "limit": 20
}
```

The server validates these options and forwards them to the configured search provider. Top-level `temporal` and per-resource `temporalByResource` clauses are compiled into per-resource date filters (resolved against the actor's `datafnTimezoneChange` history) before search executes.

## Setting Up a Search Provider

To enable local-first search on the client, configure a `searchProvider`. DataFn supports both browser-owned SearchFn and native-backed SearchFn topologies.

### Browser-Owned SearchFn Setup

The recommended browser path uses [SearchFn](https://searchfn.com) adapters via the `@searchfn/datafn-provider` bridge.
...
### Native-Backed Apple WebView Search Setup

When the same web app runs inside `WKWebView`, configure a native-backed provider instead of an IndexedDB-backed SearchFn adapter:
...
- Swift owns both the authoritative local store and the SearchFn index lifecycle.
- JavaScript must not create an IndexedDB search sidecar.
- `client.search()` and DFQL `search` blocks route through the native bridge.
- In `icloud` mode, CloudKit syncs records only; each device maintains its own derived local search index.
