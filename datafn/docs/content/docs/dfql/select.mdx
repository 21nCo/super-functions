...
When a relation endpoint is [polymorphic](/docs/dfql/schema/relations#polymorphic-relations) (declared with an array `from`/`to`), expansion resolves each related ID to its concrete resource. The expanded array can therefore contain records of **different** resource types.
...
// items: [
//   { id: "task:1", title: "...", $relation_metadata: { sortOrder: 1 } },
//   { id: "note:7", body: "...",  $relation_metadata: { sortOrder: 2 } },
// ]
```

Nested expansion across a polymorphic intermediate is validated against every concrete target resource, so `select` tokens like `items.owner.*` must resolve on all possible `items` types.
