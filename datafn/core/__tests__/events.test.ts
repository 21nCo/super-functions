...
function matchesFilter(event: DatafnEvent, filter: DatafnEventFilter): boolean {
  ...
  const fields = filter.fields === undefined
    ? undefined
...
describe("DatafnEvent and DatafnEventFilter types", () => {
  describe("TV-CORE-EVENT-001: event/filter supports action/fields/contextKeys (positive)", () => {
    ...
    it("should treat a single string field filter as exact field matching", () => {
      const event: DatafnEvent = {
        type: "mutation_applied",
        timestampMs: 1,
        fields: ["id"],
      ...
      expect(matchesFilter(event, { fields: "id" })).toBe(true);
      expect(matchesFilter(event, { fields: "ids" })).toBe(false);
    });
