...
function installNativeServerSyncHost() {
  ...
  const postMessage = vi.fn((message: unknown) => {
    ...
    void (async () => {
      const response = await handleRequest(storage, envelope, syncCalls, remoteCalls);
  ...
  return { storage, calls, syncCalls, remoteCalls, postMessage };
}
...
async function handleRequest(
  ...
  switch (envelope.method) {
    ...
    case "remote.query":
      ...
      return success(envelope, {
        ok: true,
        result: { source: "native-remote-adapter", owner: "native", data: [] },
      });
    case "remote.mutation":
      remoteCalls.push(envelope.method);
      return success(envelope, {
        ok: true,
        result: { ok: true, mutationId: "remote-mutation" },
      });
...
describe("native-backed server sync bridge integration", () => {
  afterEach(() => {
    delete (globalThis as any).window;
    vi.restoreAllMocks();
  });

  it("delegates embedded DataFn-server sync ownership to the native bridge instead of the JavaScript SyncEngine", async () => {
    const host = installNativeServerSyncHost();
    ...
    expect(host.remoteCalls).toEqual(
      expect.arrayContaining(["remote.query"]),
    );
