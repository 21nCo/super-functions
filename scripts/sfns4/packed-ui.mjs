...
export async function verifyPackedUI(candidate) {
  let root;
  try {
    if (
      !candidate ||
    ...
    throw new Error(
      "Temporary packed consumer directory is required; use a fresh sfns4-packed directory inside the OS temporary directory",
      { cause },
    );
  }
  if (
    dirname(root) !== realpathSync(tmpdir()) ||
    !/^sfns4-packed-[A-Za-z0-9]+$/.test(basename(root))
  ) {
    throw new Error(
      "Use a fresh sfns4-packed directory directly inside the OS temporary directory",
    );
  }
  writeFileSync(
...
<Accordion.Root><Accordion.Item value="one"><Accordion.Header value="one"><Accordion.Trigger value="one">Section</Accordion.Trigger></Accordion.Header><Accordion.Content value="one">Packed content</Accordion.Content></Accordion.Item></Accordion.Root>`,
  ...
  try {
    ...
    page.on("console", (event) => {
      if (
        event.type() === "error" ||
        event.text().includes("hydration_mismatch")
      )
