...
function reactStory(primitive, registry, catalog, scenarios) {
  ...
  const storyExports = scenarios.map((scenario) => `export const ${pascal(scenario)} = {
  args: { scenario: '${scenario}' },
  render: () => <StoryHarness Root={${root.exportName}} parts={parts} primitive="${primitive.id}" scenario="${scenario}" rootId="${root.id}" rootElement="${root.element}" rootProps={${JSON.stringify(scenarioProps(primitive, scenario))}} rootVoid={${root.voidElement}} />,
};`).join('\n\n');
  return stableText(`/* Generated by the uifn delivery pipeline. Real public-package story; no test doubles. */
import * as React from 'react';
import { ${imports} } from '@uifn/components-react/${primitive.id}';
import { StoryHarness } from '../harness';

const parts = ${JSON.stringify(children.map(({ id, exportName, cardinality, element, voidElement, parentId, value }) => ({ id, exportName, many: cardinality === 'many', element, voidElement, parentId, value })))}.map((part) => ({ ...part, Component: ({ ${children.map((part) => part.exportName).join(', ')} } as Record<string, unknown>)[part.exportName] }));
const meta = {
  id: 'stable-${primitive.id}',
  title: 'Stable/${primitive.name}',
  component: StoryHarness,
  tags: ['autodocs'],
  parameters: { controls: { disable: true }, uifnCompatibility: ${JSON.stringify(compatibility(primitive, registry, catalog))} },
};
export default meta;
...
function solidStory(primitive, registry, catalog, scenarios) {
  ...
  return stableText(`/* Generated by the uifn delivery pipeline. Real public-package story; no test doubles. */
import { ${imports} } from '@uifn/components-solid/${primitive.id}';
import { StoryHarness } from '../harness';
...
function svelteStory(primitive, registry, catalog, scenarios) {
  ...
  const root = parts[0];
  const children = parts.filter((part) => part !== root);
  const storyExports = scenarios.map((scenario) => `export const ${pascal(scenario)} = { args: { scenario: '${scenario}', rootProps: ${JSON.stringify(scenarioProps(primitive, scenario))} } };`).join('\n');
  return stableText(`/* Generated by the uifn delivery pipeline. Real public-package story; no test doubles. */
import { ${imports} } from '@uifn/components-svelte/${primitive.id}';
...
const meta = {
  ...
  component: StoryHarness,
  tags: ['autodocs'],
  args: { Root: ${root.exportName}, parts, primitive: '${primitive.id}', scenario: 'default', rootId: '${root.id}', rootElement: '${root.element}', rootProps: {}, rootVoid: ${root.voidElement} },
  parameters: { controls: { disable: true }, uifnCompatibility: ${JSON.stringify(compatibility(primitive, registry, catalog))} },
};
export default meta;
...
function sampleCode(primitive, registryArtifact, framework, deliveryMode) {
  ...
  if (framework === 'svelte') {
    const attributes = Object.entries(rootProps).map(([name, value]) => typeof value === 'string' ? `${name}=${JSON.stringify(value)}` : `${name}={${JSON.stringify(value)}}`).join(' ');
    return stableText(`<script lang="ts">
  import { ${root.exportName} } from '${importTarget}';
</script>
...
export function ${primitive.name}Example() {
  return React.createElement(${root.exportName}, ${JSON.stringify(rootProps)});
}`);
  }
...
export function ${primitive.name}Example() {
  return createComponent(${root.exportName}, ${JSON.stringify(rootProps)});
}`);
}
