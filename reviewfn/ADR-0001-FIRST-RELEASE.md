...
- API-key, ChatGPT and Codex Action proxy auth are supported separately; access-token and workload-identity fail explicit compatibility preflight. Compatibility is preflighted; there is no silent provider, model, auth or sandbox fallback. Raw keys are forbidden for PR review, and the bundled Action accepts only a credential-isolating proxy URL.
