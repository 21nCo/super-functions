...
`@searchfn/core` provides storage-neutral query contracts and posting codecs. `@searchfn/adapter-indexeddb` owns `IndexedDbManager`, browser availability handling, IndexedDB schema management, and resource-scoped persistence.

Object stores:

- `terms`: composite key `[resource, field, term, chunk]` with posting arrays and metrics
- `cacheState`: composite key `[resource, key]` for resource engine warm-start state
...
```
Documents → PipelineEngine → Indexer → IndexedDbAdapter persistence
                                      ↘ LruCache ↗
