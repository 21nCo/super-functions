...
await adapter.index({
  ...
  documents: [
    {
      id: "t-1",
      fields: { title: "Ship docs", description: "Write the guide" },
    },
  ],
});
...
| --------------- | ----------------- | -------------------- | ----------------------------------------------------------------------------------- |
| `dbName`        | `string`          | `"searchfn-adapter"` | Database name. Every resource is stored in this database with resource-scoped keys. |
...
1. **Indexing** — documents pass through the text pipeline (tokenize, normalize, stop words, stem). Terms and posting lists are written to IndexedDB object stores, with postings stored as delta-varint encoded chunks keyed by `[resource, field, term, chunk]`.
...
- All resources share one IndexedDB database and remain isolated through resource-prefixed keys.
