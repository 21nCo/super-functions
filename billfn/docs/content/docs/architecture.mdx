...
- Apple can cancel or revoke server-side only when Advanced Commerce is configured and the app is eligible
- Apple webhook ingestion uses App Store Server Notifications v2 plus notification history backfill
- Dodo and Apple both ingest provider webhook or notification state into the core read model
