...
const frameworkVersion = require('solid-js/package.json').version;
...
const entry = await vite.ssrLoadModule('/src/__tests__/phase-14-ssr-entry.tsx');
const allServerTrees = entry.renderPhase14SolidSsr();
const hydrationScript = entry.renderPhase14SolidHydrationScript();
if (!Array.isArray(allServerTrees) || allServerTrees.length !== 69) {
  throw new Error(`Solid SSR renderer expected 69 public trees; received ${allServerTrees?.length ?? 'invalid'}.`);
}
...
<html lang="en">
  <head><meta charset="utf-8"><title>uifn Phase 14 Solid SSR hydration</title>${hydrationScript}</head>
  <body>
    <div id="app">${tree.body}</div>
    <script id="uifn-phase-14-descriptor" type="application/json">${descriptor}</script>
    <script type="module" src="/src/__tests__/phase-14-browser-hydrate.tsx"></script>
  </body>
</html>`;
...
if (!address || typeof address === 'string') throw new Error('Solid SSR hydration server did not expose a TCP port.');
const origin = `http://127.0.0.1:${address.port}`;
const browser = await chromium.launch({ headless: true });
const observations = [];
try {
  for (const tree of serverTrees) {
    process.stdout.write(`${JSON.stringify({ framework: 'solid', mode: 'ssr-hydration', primitive: tree.primitive, status: 'running' })}\n`);
    ...
    await page.close();
    if (result?.status !== 'passed') throw new Error(`${tree.primitive} SSR hydration failed: ${JSON.stringify(result)}`);
    ...
    observations.push({ ...result, framework: 'solid', frameworkVersion, installMode, mode: 'ssr-hydration', result: 'passed' });
    process.stdout.write(`${JSON.stringify({ framework: 'solid', mode: 'ssr-hydration', primitive: tree.primitive, status: 'passed' })}\n`);
  }
  await writeFile(path.resolve(output), `${JSON.stringify(observations, null, 2)}\n`);
  console.log(JSON.stringify({ ok: true, framework: 'solid', frameworkVersion, installMode, mode: 'ssr-hydration', publicTreeCount: observations.length }));
} finally {
  await browser.close();
