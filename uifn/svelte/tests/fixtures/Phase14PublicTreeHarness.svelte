<script lang="ts">
  ...
  interface Props {
    ...
    capture: (bridge: any) => void;
  }

  let { vector, rootProps, capture }: Props = $props();
...
{#snippet rootRender(payload: import('../../lib/index.js').SveltePrimitiveRenderPayload)}
  {@const rootAction = payload.action}
  {@const captured = capture(payload.bridge)}
  {#if childlessElements.has(elementName(root.element))}
    <svelte:element
      this={elementName(root.element)}
      {...payload.props}
    ...
    >
      ...
      {:else if vector.primitive === 'Table'}
        ...
        <Table {...phase14PartProps(vector, byId('table'))}>
          ...
          <Header {...phase14PartProps(vector, byId('header'))}>
            <tr><Head {...phase14PartProps(vector, byId('head'))}>Table heading</Head></tr>
          </Header>
...
<Root {...rootProps} render={rootRender} />
