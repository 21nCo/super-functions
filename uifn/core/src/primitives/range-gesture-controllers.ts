...
export function createCarouselController(props: CarouselProps, env: UIFnEnvironment = {}): CarouselController {
  ...
  const parts: CarouselControllerParts = {
    ...
    previous: createUIFnPhase10Part('Carousel', 'previous', () => ({ role: 'button', id: id('previous'), attributes: { type: 'button' }, disabled: !count || !loop && store.getState().index === 0, on: { click: actions.previous } }), { role: true, id: true }),
    next: createUIFnPhase10Part('Carousel', 'next', () => ({ role: 'button', id: id('next'), attributes: { type: 'button' }, disabled: !count || !loop && store.getState().index === count - 1, on: { click: actions.next } }), { role: true, id: true }),
    indicatorGroup: createUIFnPhase10Part('Carousel', 'indicatorGroup', () => ({ role: 'group', id: id('indicators') }), { role: true, id: true }),
    indicator: createUIFnPhase10ValuePart('Carousel', 'indicator', (index) => ({ role: 'button', id: id('indicator', index), attributes: { type: 'button' }, aria: { current: store.getState().index === index ? 'true' : undefined, label: new Intl.NumberFormat(locale).format(index + 1) }, data: { state: store.getState().index === index ? 'active' : 'inactive' }, on: { click: () => actions.goTo(index) } }), { role: true, id: true }),
...
export function createRatingGroupController(props: RatingGroupProps = {}, env: UIFnEnvironment = {}): RatingGroupController {
  ...
  const parts: RatingGroupControllerParts = {
    ...
    hiddenInput: createUIFnPhase10Part('RatingGroup', 'hiddenInput', () => { const state = store.getState(); return { id: id('input'), tabIndex: -1, aria: { hidden: true }, style: { position: 'absolute', opacity: '0', pointerEvents: 'none', width: '1px', height: '1px' }, attributes: { type: 'radio', name: props.name, value: state.value || '', checked: state.value > 0, required: state.required, disabled: state.disabled } }; }, { id: true }),
