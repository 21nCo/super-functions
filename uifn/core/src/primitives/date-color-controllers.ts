...
export interface DatePickerState extends DateInputState { readonly open: boolean; readonly requestedOpen?: boolean; readonly visibleMonth: UIFnCalendarDate; readonly grid: ReturnType<typeof createUIFnMonthGrid>; readonly focusedDate: UIFnCalendarDate }
...
export function createDatePickerController(props: DatePickerProps = {}, env: UIFnEnvironment = {}): DatePickerController {
  ...
  const actions: DatePickerActions = {
    ...
    navigateGrid(key) { const state = store.getState(); const amount = key === 'ArrowRight' ? 1 : key === 'ArrowLeft' ? -1 : key === 'ArrowDown' ? 7 : key === 'ArrowUp' ? -7 : key === 'PageDown' ? 28 : key === 'PageUp' ? -28 : 0; if (amount === 0) return; const minimum = props.min ?? createUIFnCalendarDate(0, 1, 1); const maximum = props.max ?? createUIFnCalendarDate(9999, 12, 31); const direction = Math.sign(amount); let focused = addUIFnDateDays(state.focusedDate, amount); while (compareUIFnDates(focused, minimum) >= 0 && compareUIFnDates(focused, maximum) <= 0 && !available(focused)) { const next = addUIFnDateDays(focused, direction); if (compareUIFnDates(next, focused) === 0) { focused = state.focusedDate; break; } focused = next; } if (compareUIFnDates(focused, minimum) < 0 || compareUIFnDates(focused, maximum) > 0 || !available(focused)) focused = state.focusedDate; const visibleMonth = createUIFnCalendarDate(focused.year, focused.month, 1); store.patchState({ focusedDate: focused, visibleMonth, grid: createUIFnMonthGrid(visibleMonth, locale) }); },
