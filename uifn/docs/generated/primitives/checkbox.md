...
Canonical primitive: `checkbox`.

## Overview

<a id="overview"></a>

Checkbox is the stable styled selection-collection primitive. Behavior is owned by `@uifn/core`; framework packages adapt that behavior and the styled packages add public parts and tokens. Implementation kind: `interactive-controller`.
...
| `indicator` | `CheckboxIndicator` | `span` | one | — | `native element props`, `className?: string`, `children?: ReactNode`, `asChild?: boolean`, `render?`, `forceMount?: boolean`, `container?: HTMLElement | null`, `ref` | `native element props`, `class?: string`, `children snippet`, `render snippet`, `forceMount?: boolean`, `container?: HTMLElement | null`, `bind:ref` | `native element props`, `class?: string`, `children?: JSX.Element`, `render?`, `forceMount?: boolean`, `container?: HTMLElement | null`, `ref` |
| `label` | `CheckboxLabel` | `span` | one | — | `native element props`, `className?: string`, `children?: ReactNode`, `asChild?: boolean`, `render?`, `forceMount?: boolean`, `container?: HTMLElement | null`, `ref` | `native element props`, `class?: string`, `children snippet`, `render snippet`, `forceMount?: boolean`, `container?: HTMLElement | null`, `bind:ref` | `native element props`, `class?: string`, `children?: JSX.Element`, `render?`, `forceMount?: boolean`, `container?: HTMLElement | null`, `ref` |
...
- Core: `@uifn/core/checkbox: createCheckboxController(props, environment?)`
- State: `CheckboxState`
- Actions: `CheckboxActions`
- Parts: `CheckboxController["parts"]`
- DOM owner: @uifn/dom owns root-scope-modality-tabbability, form-bridges-live-regions
- react context: `CheckboxProvider and useCheckbox(inputs); adapter context remains private`
- svelte context: `CheckboxProvider; adapter context remains private to compound descendants`
- solid context: `CheckboxProvider; adapter context remains private to compound descendants`
...
- `{ type: "TOGGLE"; key?: string; inputModality?: "keyboard" | "pointer" | "touch" | "virtual" }` — Checkbox semantic transition event TOGGLE. Source: controller-or-native-contract.
- `{ type: "SET_CHECKED"; checked: boolean | "indeterminate" }` — Checkbox semantic transition event SET_CHECKED. Source: controller-or-native-contract.
- `{ type: "FORM_RESET"; form?: string }` — Checkbox semantic transition event FORM_RESET. Source: controller-or-native-contract.

Controlled callbacks:

- `onCheckedChange(value: boolean|string) => void` — Called after Checkbox requests a checked change. Controlled consumers must commit the value back through checked.

Root inputs and defaults:

| Input | Type | Required | Reactive | Default | Description |
|---|---|---:|---:|---|---|
| `checked` | `boolean|string` | no | yes | `undefined (no public prop override)` | Optional reactive checked input from the canonical Checkbox contract. |
| `defaultChecked` | `boolean|string` | no | yes | `undefined → false` | Optional reactive defaultChecked input from the canonical Checkbox contract. |
| `name` | `string` | no | yes | `undefined (no public prop override)` | Optional reactive name input from the canonical Checkbox contract. |
| `value` | `string` | no | yes | `undefined (uncontrolled)` | Optional reactive value input from the canonical Checkbox contract. |
| `disabled` | `boolean` | no | yes | `undefined → false` | Optional reactive disabled input from the canonical Checkbox contract. |
| `readOnly` | `boolean` | no | yes | `undefined → false` | Optional reactive readOnly input from the canonical Checkbox contract. |
| `required` | `boolean` | no | yes | `undefined → false` | Optional reactive required input from the canonical Checkbox contract. |
...
Use the compound root `Checkbox` or named parts shown above. Import `@uifn/components/styles.css` once, then override tokens or low-specificity part selectors in a later CSS layer.

Stable data attributes:

- `data-uifn-component="checkbox"` on all styled parts; stable.
- `data-uifn-part="root | control | indicator | label | hiddenInput"` on all styled parts; stable.
- `data-state="unchecked | checked | indeterminate"` on stateful parts; stable semantic state.
...
|---|---|---|
| react | `@uifn/components-react/checkbox` | `components/uifn/react/checkbox.ts` |
| svelte | `@uifn/components-svelte/checkbox` | `components/uifn/svelte/checkbox/CheckboxControl.svelte` |
| solid | `@uifn/components-solid/checkbox` | `components/uifn/solid/checkbox.ts` |
...
Use `uifn add checkbox --framework <react|svelte|solid>` through `@uifn/registry`. The lockfile records the catalog, generator, template, and output hashes. Source-install files have the same public component contract as package delivery; edit intentionally and let `uifn diff` report local divergence.
