...
The host mounts this framework-neutral router under its chosen base path. It exposes `POST /v1/memories` with `content`, optional `containerTags`, and optional `metadata`; and `POST /v1/memories/search` with `q`, optional tags, filters, limit, and threshold. Body tags add constraints to the trusted host tags. Inputs are strict: a body `tenantId` is rejected, as are unknown fields. The limit is 1–100 and threshold is -1 to 1. Missing authorization returns 401; invalid input returns 400. The default body cap is one MiB.
