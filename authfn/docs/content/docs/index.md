...
- **Type-safe end to end.** `@authfn/core` (Node), `@authfn/client` (browsers/Node), `@authfn/svelte` (Svelte stores), `authfn` on PyPI, and `AuthFnSwift` on iOS/macOS speak the same envelopes, error codes, and routes.
...
If you have **15 minutes**, follow the [Getting Started](./getting-started) tutorial — it runs through `createAuthFn`, plugin enablement, mounting on Hono, and signing in a user from the typed client.
...
| --- | --- | --- |
| Cookie sessions | Browser sessions with rotation, idle/absolute timeouts, CSRF double-submit, multi-device session listing and revoke | bundled in `@authfn/core` |
| Token sessions | Bearer-token sessions for non-browser clients (mobile, CLI, server-to-server) | bundled in `@authfn/core` |
| Email & password | Sign-up/sign-in, password reset via OTP, optional Have-I-Been-Pwned integration | `authFnPasswordPlugin` |
| Email OTP | One-time codes for email verification, sign-in, sign-up, and password reset | `authFnEmailOtpPlugin` |
| Social OAuth | Google, Apple, GitHub — and any custom provider via the resolver hook | `authFnSocialOAuthPlugin` |
| API keys | User-owned API keys with scopes, named, revocable, securely hashed | `authFnApiKeyPlugin` |
| Two-factor (TOTP) | RFC 6238 TOTP with recovery codes, time-window tolerance, and pluggable encryption | `authFnTwoFactorPlugin` |
| Multi-region | Region-aware lookup, runtime overlays (cookie domain, OAuth, baseUrl, issuer per region), wrong-authority correction | `authFnMultiRegionPlugin` |
| Native handoff | Web ↔ native session handoff for iOS/Android wrappers | `authFnNativeHandoffPlugin` |
| Admin API | List/delete users with custom authorization | `@authfn/admin` |
| OpenAPI | Auto-generated spec from your enabled plugins, served from the kernel | bundled in `@authfn/core` |

## SDK matrix

| SDK | Package | Server / Client | Status |
| --- | --- | --- | --- |
| Server kernel (Node) | `@authfn/core` | Server | Stable |
...
- [SDKs](./sdk) — `@authfn/core`, `@authfn/client`, `@authfn/svelte`, the Python package, and Swift.
