...
import { accountSettingsSchema } from '../auth.js';

const DEFAULT_DATABASE_URL = 'postgres://postgres:postgres@127.0.0.1:5432/authfn_examples_account_settings';

const dateFieldsByModel = new Map<string, string[]>(
  accountSettingsSchema.schemas.map((table) => [
    table.modelName,
...
export interface AccountSettingsDatabase {
  adapter: Adapter;
...
export function createAccountSettingsDatabase(
  databaseUrl: string = process.env.DATABASE_URL ?? DEFAULT_DATABASE_URL
): AccountSettingsDatabase {
