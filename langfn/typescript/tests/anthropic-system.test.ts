...
it.each([['required', 'any'], ['none', 'none'], ['auto', 'auto']])('preserves system messages and maps %s tool choice', async (choice, expected) => {
  const fetch = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => new Response(JSON.stringify({ id: 'm', content: [{ type: 'text', text: 'ok' }], usage: { input_tokens: 1, output_tokens: 1 } }), { status: 200 }));
