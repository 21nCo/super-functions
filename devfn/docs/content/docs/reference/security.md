...
description: Manifest trust, secret handling, local exposure, and ownership.
---
...
- Runtime children inherit a small base environment plus explicit `envAllowlist` entries. Sensitive-looking keys must also be listed in `secretEnv`; a detached wrapper redacts those effective values before native-process logs are persisted. Literal values are rejected only when their key looks sensitive, so review every manifest literal. Compose logging is disabled for services with `secretEnv` because Docker otherwise retains historical unredacted logs. Inherited `secretEnv` values are not persisted in the registry or lifecycle receipt. Managed-process command arguments are serialized verbatim in receipts, so never embed credentials in command arguments; supply them through the explicitly allowed secret environment instead.
