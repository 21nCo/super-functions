...
- A `devfn.config.ts` or JavaScript manifest is executable repository code. DevFn refuses to load it until the user records trust for its exact digest with `--trust`, then executes a private copy of those exact verified bytes. Executable manifests must be self-contained; imports and `require()` are rejected so untrusted dependency files cannot bypass the digest boundary.
