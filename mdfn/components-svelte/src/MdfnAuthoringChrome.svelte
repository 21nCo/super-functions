...
<section data-mdfn-component="authoring-chrome" data-compact={model.compact ? 'true' : 'false'} aria-label="Markdown authoring controls">
  ...
  {#if !effectiveReadOnly && model.bubbleVisible}<div data-mdfn-surface="bubble-toolbar"><MdfnToolbar {controller} groups={model.bubble.groups} {commandTarget} ariaLabel="Selection formatting" /></div>{/if}
  {#if !effectiveReadOnly && model.floatingVisible}<div data-mdfn-surface="floating-toolbar"><MdfnToolbar {controller} groups={model.floating.groups} {commandTarget} ariaLabel="Block formatting" /></div>{/if}
  ...
  <CardRoot data-mdfn-surface="editorial"><CardHeader><CardTitle>Review</CardTitle></CardHeader><CardContent><p>State: {model.reviewState}</p>{#if !effectiveReadOnly}<InputRoot aria-label="Comment" value={comment} oninput={(event: Event) => comment = (event.currentTarget as HTMLInputElement).value} /><ButtonRoot type="button" disabled={!comment.trim()} onclick={addComment}>Add comment</ButtonRoot><InputRoot aria-label="Suggestion replacement" value={suggestion} oninput={(event: Event) => suggestion = (event.currentTarget as HTMLInputElement).value} /><ButtonRoot type="button" onclick={addSuggestion}>Add suggestion</ButtonRoot>{/if}<ul aria-label="Comments">{#each model.comments ?? [] as thread (thread.id)}<li>{#each thread.messages as message (message.id)}<p>{message.body}</p>{/each}{thread.resolved ? 'Resolved' : 'Open'} {#if !effectiveReadOnly}<InputRoot aria-label={`Reply to comment ${thread.id}`} value={replies[thread.id] ?? ''} oninput={(event: Event) => replies[thread.id] = (event.currentTarget as HTMLInputElement).value} /><ButtonRoot type="button" disabled={!(replies[thread.id] ?? '').trim()} onclick={() => { updateSidecar(replyToComment({ sidecar: controller.getState().sidecar ?? {}, threadId: thread.id, body: replies[thread.id] ?? '', actor: currentActor }), 'editorial:comment-reply'); replies[thread.id] = ''; }}>Reply</ButtonRoot><ButtonRoot type="button" onclick={() => updateSidecar(setCommentResolved({ sidecar: controller.getState().sidecar ?? {}, threadId: thread.id, resolved: !thread.resolved, actor: currentActor }), 'editorial:comment-resolution')}>{thread.resolved ? 'Reopen' : 'Resolve'}</ButtonRoot>{/if}</li>{/each}</ul><ul aria-label="Suggestions">{#each model.suggestions ?? [] as entry (entry.id)}<li>{entry.replacement} ({entry.status}) {#if entry.status === 'pending' && !effectiveReadOnly}<ButtonRoot type="button" onclick={() => decideSuggestion({ controller, suggestionId: entry.id, decision: 'accepted', actor: currentActor })}>Accept</ButtonRoot><ButtonRoot type="button" onclick={() => decideSuggestion({ controller, suggestionId: entry.id, decision: 'rejected', actor: currentActor })}>Reject</ButtonRoot>{/if}</li>{/each}</ul>{#if !effectiveReadOnly}<div aria-label="Review transitions">{#each ['draft', 'in-review', 'changes-requested', 'approved'] as state}<ButtonRoot type="button" disabled={state === model.reviewState || !canTransitionReview(model.reviewState, state as ReviewState)} onclick={() => setReview(state as ReviewState)}>{state}</ButtonRoot>{/each}</div>{/if}</CardContent></CardRoot>
  <CardRoot data-mdfn-surface="history"><CardHeader><CardTitle>Version history</CardTitle></CardHeader><CardContent><ol aria-label="Document versions">{#each versions as entry (entry.version)}<li>Version {entry.version}{entry.authorId ? ` by ${entry.authorId}` : ''} {#if onRestoreVersion && !effectiveReadOnly}<ButtonRoot type="button" onclick={() => void onRestoreVersion?.(entry.version)}>Restore</ButtonRoot>{/if}</li>{/each}</ol><ol aria-label="Editorial activity">{#each model.audit ?? [] as entry (entry.id)}<li>{entry.action} by {entry.actorId}</li>{/each}</ol></CardContent></CardRoot>
</section>
