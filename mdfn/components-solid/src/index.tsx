...
export const MdfnEditorShell: Component<MdfnEditorShellProps> = (props) => {
  ...
  return <div class={props.class} data-mdfn-component="editor-shell">
    {!props.hideToolbar && !props.readOnly && <MdfnToolbar controller={props.controller} groups={props.toolbarGroups} commandTarget={commandTarget()} />}
    {!props.hideAuthoringChrome && <MdfnAuthoringChrome controller={props.controller} editor={editor()} mode={props.mode} readOnly={props.readOnly} actor={props.actor} onSelectFiles={props.onSelectFiles} onModeChange={props.onModeChange} versions={props.versions} onRestoreVersion={props.onRestoreVersion} />}
    <MdfnEditor
      ...
      onFiles={async (files) => { const markdown = await props.onSelectFiles?.(files); if (markdown) insertMarkdownAtSelection(props.controller, editor(), markdown); await props.onFiles?.(files); }}
      editorRef={setEditor}
    />
  </div>;
};
...
export const MdfnAuthoringChrome: Component<MdfnAuthoringChromeProps> = (props) => {
  ...
  return <section data-mdfn-component="authoring-chrome" data-compact={model().compact ? "true" : "false"} aria-label="Markdown authoring controls">
    ...
    <Show when={!props.readOnly && model().bubbleVisible}><div data-mdfn-surface="bubble-toolbar"><MdfnToolbar controller={props.controller} groups={model().bubble.groups} commandTarget={commandTarget()} ariaLabel="Selection formatting" /></div></Show>
    <Show when={!props.readOnly && model().floatingVisible}><div data-mdfn-surface="floating-toolbar"><MdfnToolbar controller={props.controller} groups={model().floating.groups} commandTarget={commandTarget()} ariaLabel="Block formatting" /></div></Show>
    <Show when={!props.readOnly}><Button type="button" aria-expanded={model().slashOpen} aria-controls="mdfn-insert-menu" onClick={() => setSlashOpen((value) => !value)}>Insert</Button></Show>
    <Show when={!props.readOnly && model().slashOpen}><Card id="mdfn-insert-menu" data-mdfn-surface="slash-menu"><Card.Header><Card.Title>Insert</Card.Title></Card.Header><Card.Content><Input aria-label="Filter insert commands" value={slashQuery()} onInput={(event) => setSlashQuery(event.currentTarget.value)} /><div role="listbox" aria-label="Insert commands"><For each={model().slashCommands}>{(item) => <Button type="button" role="option" onClick={() => runSlash(item)}>{item.label}</Button>}</For></div></Card.Content></Card></Show>
    <Show when={!props.readOnly}><Card data-mdfn-surface="link-editor"><Card.Header><Card.Title>Link</Card.Title></Card.Header><Card.Content><Input type="url" aria-label="Link URL" value={link()} onInput={(event) => setLink(event.currentTarget.value)} /><Button type="button" onClick={() => props.editor?.setLink(link())}>Apply link</Button><Button type="button" onClick={() => props.editor?.removeLink()}>Remove link</Button></Card.Content></Card></Show>
    ...
    <Show when={!props.readOnly}><Card data-mdfn-surface="file-controls"><Card.Header><Card.Title>Files</Card.Title></Card.Header><Card.Content><Input type="file" aria-label="Select files" multiple onChange={(event) => { const files = [...(event.currentTarget.files ?? [])]; void props.onSelectFiles?.(files).then((markdown) => { if (markdown) insertMarkdownAtSelection(props.controller, props.editor, markdown); }); }} /></Card.Content></Card></Show>
    ...
    <Card data-mdfn-surface="editorial"><Card.Header><Card.Title>Review</Card.Title></Card.Header><Card.Content><p>State: {model().reviewState}</p><Show when={!props.readOnly}><Input aria-label="Comment" value={comment()} onInput={(event) => setComment(event.currentTarget.value)} /><Button type="button" disabled={!comment().trim()} onClick={addComment}>Add comment</Button><Input aria-label="Suggestion replacement" value={suggestion()} onInput={(event) => setSuggestion(event.currentTarget.value)} /><Button type="button" onClick={addSuggestion}>Add suggestion</Button></Show><ul aria-label="Comments"><For each={model().comments ?? []}>{(thread) => <li><For each={thread.messages}>{(message) => <p>{message.body}</p>}</For>{thread.resolved ? "Resolved" : "Open"} <Show when={!props.readOnly}><Input aria-label={`Reply to comment ${thread.id}`} value={replies()[thread.id] ?? ""} onInput={(event) => setReplies((current) => ({ ...current, [thread.id]: event.currentTarget.value }))} /><Button type="button" disabled={!(replies()[thread.id] ?? "").trim()} onClick={() => { updateSidecar(replyToComment({ sidecar: props.controller.getState().sidecar ?? {}, threadId: thread.id, body: replies()[thread.id] ?? "", actor: actor() }), "editorial:comment-reply"); setReplies((current) => ({ ...current, [thread.id]: "" })); }}>Reply</Button><Button type="button" onClick={() => updateSidecar(setCommentResolved({ sidecar: props.controller.getState().sidecar ?? {}, threadId: thread.id, resolved: !thread.resolved, actor: actor() }), "editorial:comment-resolution")}>{thread.resolved ? "Reopen" : "Resolve"}</Button></Show></li>}</For></ul><ul aria-label="Suggestions"><For each={model().suggestions ?? []}>{(entry) => <li>{entry.replacement} ({entry.status}) <Show when={entry.status === "pending" && !props.readOnly}><Button type="button" onClick={() => decideSuggestion({ controller: props.controller, suggestionId: entry.id, decision: "accepted", actor: actor() })}>Accept</Button><Button type="button" onClick={() => decideSuggestion({ controller: props.controller, suggestionId: entry.id, decision: "rejected", actor: actor() })}>Reject</Button></Show></li>}</For></ul><Show when={!props.readOnly}><div aria-label="Review transitions"><For each={["draft", "in-review", "changes-requested", "approved"] as const}>{(state) => <Button type="button" disabled={state === model().reviewState || !canTransitionReview(model().reviewState, state)} onClick={() => setReview(state)}>{state}</Button>}</For></div></Show></Card.Content></Card>
    <Card data-mdfn-surface="history"><Card.Header><Card.Title>Version history</Card.Title></Card.Header><Card.Content><ol aria-label="Document versions"><For each={props.versions ?? []}>{(entry) => <li>Version {entry.version}{entry.authorId ? ` by ${entry.authorId}` : ""} <Show when={props.onRestoreVersion && !props.readOnly}><Button type="button" onClick={() => void props.onRestoreVersion?.(entry.version)}>Restore</Button></Show></li>}</For></ol><ol aria-label="Editorial activity"><For each={model().audit ?? []}>{(entry) => <li>{entry.action} by {entry.actorId}</li>}</For></ol></Card.Content></Card>
  </section>;
};
