...
Applications that need a control plane can also use the separate `hostfn/operator` service and `@hostfn/admin` capability to manage scoped targets, deployments, domains, and environment-variable keys. See [Administration](/docs/documentation/advanced/administration) for the host-owned storage, executor, authorization, and audit boundaries.
