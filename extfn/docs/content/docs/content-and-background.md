...
# Content and background handlers

The config can declare several content scripts, each with its own ID, entry, match patterns, anchors, mount mode, and style isolation. Anchor kinds include `selector`, `selector-list`, and `resolver`; mount modes include `append`, `prepend`, `replace`, and `shadow`. The content helpers manage anchored mounting and reinjection.
