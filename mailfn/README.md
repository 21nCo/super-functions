...
3. Separate Queue bindings process MIME parsing and durable webhook delivery. The parse consumer writes attachment bytes to R2, normalizes content into D1, and resolves the thread; inbound acceptance persists events and pending deliveries before enqueueing webhook work, so endpoint latency never blocks MIME parsing.
