...
MailFn stores original MIME in object storage before the message row and parse job. The parse consumer normalizes content, writes attachment bytes, and resolves threading. The typed client exposes message list, read, search, raw download, attachments, labels, threads, and explicit OTP/link extraction. Raw and attachment downloads return `Uint8Array`.
