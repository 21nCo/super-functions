...
it("fails closed when final JSON composition reconstructs a target credential", () => {
  const secret = '","';
  const client = new McpFnClient({
    target: customTarget({
      kind: "custom",
...
it("fails closed when final snapshot JSON reconstructs a target credential", async () => {
  const secret = '","';
  const client = new McpFnClient({
    target: customTarget({
      kind: "custom",
  ...
  await expect(inspector.snapshot())
