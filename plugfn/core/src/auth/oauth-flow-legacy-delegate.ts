...
function toOAuthProviderDescriptor(config: OAuth2Config): OAuthProviderDescriptor {
  ...
  return {
    ...
    tokenBodyEncoding: config.tokenBodyEncoding,
    tokenHeaders: config.tokenHeaders,
    defaultScopes: [...config.scopes],
    supportsPkce: config.supportsPkce ?? true,
    supportsRefreshToken: config.supportsRefreshToken ?? true,
    scopeParameter: config.scopeParameter,
    authorizationCodeTokenPath: config.authorizationCodeTokenPath,
